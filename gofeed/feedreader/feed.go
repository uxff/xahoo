		...
		if feed.Items[i] != nil {
			...
			for _, imgLink := range imgLinks {
				...
				for _, attrElem := range attrRets {
					...
					if len(attrElem) > 2 {
						fmt.Sscanf(attrElem[2], "%d", &attrVal)
					}
				}
				if attrVal < 10 {
					fmt.Println("the image seemes like ad:", imgLink[0])
					continue
				}
				feed.Items[i].ImgLinks = append(feed.Items[i].ImgLinks, &imgLink[1])
			}
