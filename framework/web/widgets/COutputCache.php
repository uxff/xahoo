...
class COutputCache extends CFilterWidget
{
	...
	const QJS_PREFIX_CACHE = 'qianjins';
	/**
	...
	protected function checkContentCache()
	{
			...
			&& ($this->_cache=$this->getCache())!==null)
		{
			if($this->duration>0 && ($data=$this->_cache->get($this->getCacheKey()))!==false)
			{
                ...
                file_put_contents('/tmp/cache_log',var_export($_SERVER['REQUEST_URI'],true)."\n======hit=======\n",FILE_APPEND);
				return true;
			}
			if($this->duration==0)
	...
	protected function getCacheKey()
	{
		...
		else
		{
			...
			if($this->varyBySession)
            {
				...
				Yii::app()->getSession();
                $sessionKey = md5(serialize($_SESSION));
				$key.= $sessionKey;//Yii::app()->getSession()->getSessionID();
            }
			$key.='.';
			...
			if($this->varyByRequestUri)
				$key.= md5($_SERVER['REQUEST_URI']);
			$key.='.';


            file_put_contents('/tmp/cache_log',"Key:==>".$key."\n========\n",FILE_APPEND);
