...
class CController extends CBaseController {
        ...
        public function processOutput($output) {
                Yii::app()->getClientScript()->render($output); //develope 输出页面的js
    ...
    */
    ...
    /*
        返回固定格式的错误
    */
    public function jsonError($msg, $value = array(), $code = 0) {
    ...
    */
    public function jsonSuccess($msg, $value = array()) {
        return $this->jsonData($msg, 1, $value);
    }
    /*
        强制下载文件
        @param $filename    服务器本地文件名
        @param $name        下载显示文件名
    */
    public function forceDownload($filename, $name='') { 
     
        if (false == file_exists($filename)) { 
            //return false; 
            throw new CHttpException(404, '文件不存在：'.$filename);
        } 
