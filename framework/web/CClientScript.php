...
class CClientScript extends CApplicationComponent {
         ...
         */
        public function render(&$output, $document = 0) {//develope 输出页面的js初始化 &$output=>&$output, $document = 0
                if (!$this->hasScripts)
                        return;
                ...
                if ($this->enableJavaScript) {
                        ...
                        $this->renderBodyEnd($output, $document);//develope 输出页面的js初始化 $output=>$output, $document
                }
        }
         ...
         * @param string $output the output to be inserted with scripts.
         */
        public function renderBodyEnd(&$output, $document = 0) {//develope 输出页面的js初始化 &$output=>&$output, $document = 0
                ...
                if (isset($this->scripts[self::POS_READY])) {
                        if ($fullPage || $document)//develope 输出页面的js初始化 $fullPage=>$fullPage || $document
