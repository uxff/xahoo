...
abstract class CActiveRecord extends CModel {
         ...
         */
        //default column modify by xingkun 20141028

        public function ares_register_behaviors() {
                $behaviors = array(
                    'CTimestampBehavior' => array(
                        'class' => 'zii.behaviors.CTimestampBehavior',
                        'createAttribute' => 'create_time', //创建时间
                        'updateAttribute' => 'last_modified', //更新时间
                    )
                );
        ...
        public function __construct($scenario = 'insert') {
                ...
                $this->afterConstruct();
                if ($this->tableSchema) {
                        foreach ($this->tableSchema->columns as $column_name => $column) {
                                if (!in_array($column_name, array('create_time', 'last_modified'))) {
                                        $this->FormElements[$column_name] = array(
                                            'name' => $column->name,
                                            'type' => $column->type,
                                        ...
                                        if ($column->comment) {
                                                if (stripos($column->comment, '>') !== false) {
                                                	    //php 5.3 split这个函数在新版本不支持了 split改为 explode；如果是正则表达式，split改为preg_split
                                                        list($comment, ) = explode(">", str_replace(array(">", "》"), ">", $column->comment));
                                                } else {
                                                        $comment = $column->comment;
                                                }
                                                $this->FormElements[$column_name]['comment'] = $comment;
                                        } else {
                                                $this->FormElements[$column_name]['comment'] = ucwords(str_replace("_", " ", $column->name));
                                        }
                                }
                        }
                }
