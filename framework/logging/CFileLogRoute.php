...
class CFileLogRoute extends CLogRoute
{
	...
	{
		parent::init();
		if(Yii::app()->params['LOG_PATH']!=null && file_exists(Yii::app()->params['LOG_PATH'])) {
			$this->setLogPath(Yii::app()->params['LOG_PATH']);
        } else {
			$this->setLogPath(Yii::app()->getRuntimePath());
        }
		if(Yii::app()->params['LOG_NAME']!=null) {
			$this->setLogFile(Yii::app()->params['LOG_NAME']);
        }
	}
