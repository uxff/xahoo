...
(function() {
  var Circles = window.Circles = function(options) {
    ...
    this._toptext        = options.toptext; // #进度
    this._number         = options.number || this._percentage;
    this._strokeWidth    = options.width  || 10;
    this._colors         = options.colors || ['#EEE', '#F00'];
    this._toptextStyle   = options.toptextStyle || 'font-size:14px;line-height:0px;padding-top:25px'; //进度class
    this._interval       = 16;
  ...
  Circles.prototype = {
    ...
    _generateText: function() {
      var html =  '<div class="' + this._textWrpClass + '" style="position:absolute; top:0; left:0; text-align:center; width:100%;' +
        ' font-size:' + this._radius * .5 + 'px; height:' + this._svgSize + 'px;';
      if (this._toptext) {
      	html += ' line-height:' + this._svgSize/3 + 'px;"';
      } else {
      	html += ' line-height:' + this._svgSize + 'px;"';
      }
    ...
    _calculateNumber: function(number) {
      ...
      if (this._toptext) {
      	html +='<p style="' + this._toptextStyle + '">' + this._toptext + '</p>';
      }
      html += parts[0];
      if (parts.length > 1) {
