...
/// 发红包相关代码
public function sendRedPack($arrInfo)
{
/*
$arrInfo = array(
        'nonce_str'=>'',//随机字符串,32位
...
$arrInfo = array(
        'nonce_str'=>'ljeUjFDR8QMRuuZe',//$nonce_str,//随机字符串,32位
        'mch_billno'=>'qjs_20150709_21:30:23_'.rand(1000,9999),// 商户订单号 28位
        'mch_id'=>'1241495602 ', // 商户号
        //'sub_mch_id'=>'', //子商户号(选填)
        'wxappid'=>'wx7345a7e7764a9f88', //公众号(服务号)appid
        'nick_name'=>'仟金所', //红包提供方名
        'send_name'=>'仟金所', //红包发送者名称
        're_openid'=>$this->getRevFrom(), //接收红包的用户
        'total_amount'=>'100', //付款金额,单位分
        'min_value'=>'100', //最小红包金额
        'max_value'=>'100', //最大红包金额
        'total_num'=>'1', //红包发送总人数
        'wishing'=>'恭喜发财', //红包祝福语 128
        'client_ip'=>'192.168.1.1', //调用接口的机器ip地址 15
        'act_name'=>'关注仟金所', //活动名称 32
        'remark'=>'这是备注,备注,备注,备注', //备注 256
        //'logo_imgurl'=>'', //商户logo 选填 128
        //'share_content'=>'', //分享文案 选填 256
        //'share_url'=>'', //分享链接 选填 128
        //'share_imgurl'=>'', //分享图片url 选填 128
...
file_put_contents('/tmp/debug',var_export($stringA,true),FILE_APPEND);
$stringA = $stringA."&key=".trim($privatekey);
file_put_contents('/tmp/debug',var_export($stringA,true),FILE_APPEND);
$sign = strtoupper(md5($stringA));
$arrInfo['sign'] = $sign;
file_put_contents('/tmp/debug',var_export($arrInfo,true),FILE_APPEND);

$redPackUrl = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack';
$postXml = $this->arrayToXml($arrInfo);
file_put_contents('/tmp/debug',var_export($postXml,true),FILE_APPEND);
$data = $this->postRedPack($redPackUrl,$postXml);
file_put_contents('/tmp/debug',var_export($data,true),FILE_APPEND);
/*
<xml>
    <sign></sign>
...
function postRedPack($url, $vars, $second=30,$aHeader=array())
{
        ...
        curl_setopt($ch,CURLOPT_SSLCERT,dirname(__FILE__).'/zhengshu/apiclient_cert.pem');
        curl_setopt($ch,CURLOPT_SSLKEY,dirname(__FILE__).'/zhengshu/apiclient_key.pem');
        curl_setopt($ch,CURLOPT_CAINFO,dirname(__FILE__).'/zhengshu/rootca.pem');
