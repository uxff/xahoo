<?php
class StasticByDayModel extends StasticByDayModelBase
{
        ...
        public function mySearch($condition = [])
        {
            ...
            if (isset($condition['time_start']) && !empty($condition['time_start'])) {
                $criteria->addCondition('date>=:start_day');
                $criteria->params[':start_day'] = date('Y-m-d', strtotime($condition['time_start']));
            }
            if (isset($condition['time_end']) && !empty($condition['time_end'])) {
                $criteria->addCondition('date<=:end_day');
                $criteria->params[':end_day'] = date('Y-m-d', strtotime($condition['time_end']));
            }
               // @todo Please modify the following code to remove attributes that should not be searched.
                ...
                $list = $this->orderBy('date desc')->findAll($criteria);
                $pages = array(
         ...
         * @return StasticByDayModel the static model class
         */
    ...
    static public function searchForReport($condition = [])
    {
        ...
        if (isset($condition['time_start']) && !empty($condition['time_start'])) {
            ...
            $whereParam  [':start_day'] = date('Y-m-d', strtotime($condition['time_start']));
        }
        if (isset($condition['time_end']) && !empty($condition['time_end'])) {
            $whereClouse []= 'date<=:end_day';
            $whereParam  [':end_day'] = date('Y-m-d', strtotime($condition['time_end']));
        }

        $ret = Yii::app()->db->createCommand()
            ->select('id, date, pv, uv, share_count, reg_count, xqsj_uv,points_add,points_consume')
            ->from('fh_stastic_by_day')
            ->where(implode(' and ', $whereClouse), $whereParam)
            ->limit(1000)
            ->order('date desc')
            ->queryAll();
        return $ret;
