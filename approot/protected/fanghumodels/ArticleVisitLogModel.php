<?php
class ArticleVisitLogModel extends ArticleVisitLogModelBase
{
         ...
         * @return ArticleVisitLogModel the static model class
         */
        ...
        /*
            按起始时间统计数据
        */
        static public function stasticVisit($aid, $startTime, $endTime) {
            $ret = Yii::app()->db->createCommand()
                ->select('article_id, count(visitor_ip_u) pv, count(distinct visitor_ip_u) uv')
                ->from('fh_article_visit_log')
                ->where('article_id=:aid and create_time>=:start_time and create_time<=:end_time', [
                    ':aid'          => $aid,
