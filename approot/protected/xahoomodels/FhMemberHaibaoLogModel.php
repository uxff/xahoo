<?php
class FhMemberHaibaoLogModel extends FhMemberHaibaoLogModelBase
{
        ...
        public function relations() {
                $curRelations = array(
                    'haibao' => array(self::HAS_ONE, 'FhMemberHaibaoModel', '', 'on' => 'haibao.sns_bind_id=t.sns_bind_id'),
         ...
         * @return FhMemberHaibaoLogModel the static model class
         */
    ...
    static public function countMax($sceneid) {
        //FhMemberHaibaoLogModel::model()->with('poster')->findAll();
        $sql = 'select min(poster.lowest_withdraw_sum) themin, sum(poster.highest_withdraw_sum) themax from fh_member_haibao_log t, fh_poster poster where t.poster_id = poster.id and sns_bind_id=:mid';
        $ret = 
        Yii::app()->db->createCommand()
        ->from('fh_member_haibao_log t')
        ->select('min(poster.lowest_withdraw_sum) themin, sum(distinct poster.highest_withdraw_sum) themax')
        ->join('fh_poster poster', 't.poster_id = poster.id')
        ->where('t.sns_bind_id=:mid', [':mid'=>$sceneid])
