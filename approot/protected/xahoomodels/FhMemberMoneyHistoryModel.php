<?php
class FhMemberMoneyHistoryModel extends FhMemberMoneyHistoryModelBase
{
    const TYPE_REWARD   = 1;
    const TYPE_WITHDRAW = 2;
    static $ARR_TYPE = [
        self::TYPE_REWARD   => '奖励获得',
        self::TYPE_WITHDRAW => '提现',
    ];
	
    const EID_SUBSCRIBE_FH = 'WX_ADD_FH';
        ...
        public function mySearch2($condition = [])
        {
           // @todo Please modify the following code to remove attributes that should not be searched.
            ...
            if (isset($condition['member_id']) && !empty($condition['type'])) {
                $criteria->addCondition('member_id = '.$condition['member_id'].' and type='.$condition['type']);
            }
         ...
         * @return FhMemberMoneyHistoryModel the static model class
         */
