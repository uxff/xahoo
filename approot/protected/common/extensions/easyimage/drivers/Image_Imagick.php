...
class Image_Imagick extends Image
{
	...
	protected function _do_resize($width, $height, $bg_width, $bg_height)
	{
		if ($this->im->scaleImage($width, $height)) {
			...
			$color = sprintf('rgb(%d, %d, %d)', 255, 255, 255);

			// Create a new image for the background
			$background = new Imagick();
			$background->newImage($bg_width, $bg_height, new ImagickPixel($color));

			// Calculate foregroud location
			$offset_x = ($bg_width > $width) ? round(($bg_width - $width) / 2) : 0;
			$offset_y = ($bg_height > $height) ? round(($bg_height - $height) / 2) : 0;

			// auto add padding
			if($background->compositeImage($this->im, Imagick::COMPOSITE_DISSOLVE, $offset_x, $offset_y)) {
				$this->im = $background;
			}
