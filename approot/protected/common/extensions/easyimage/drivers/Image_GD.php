...
class Image_GD extends Image
{
	 ...
	 * @param   integer  $height  backgroud width
	 * @param   integer  $height  backgroud height
	 * 
	 * @return  void
	 */
	protected function _do_resize($width, $height, $bg_width, $bg_height)
	{
		...
		// Create the temporary image to copy to
		$image = $this->_create($bg_width, $bg_height);

		// Calculate foregroud location
		$x = ($bg_width > $width) ? round(($bg_width - $width) / 2) : 0;
		$y = ($bg_height > $height) ? round(($bg_height - $height) / 2) : 0;

		// Execute the resize
		if (imagecopyresampled($image, $this->_image, $x, $y, 0, 0, $width, $height, $pre_width, $pre_height)) {
			// Swap the new image for the old one
			imagedestroy($this->_image);
			$this->_image = $image;
