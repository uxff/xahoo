...
class Image
{
	 ...
	 * @param   boolean  $padding auto add white padding or not
	 * @return  $this
	...
	public function resize($width = NULL, $height = NULL, $master = NULL, $padding = true)
	{
		...
		// Auto fill white padding or not
		if ($padding === TRUE) {
			$this->_do_resize($width, $height, $bg_width, $bg_height);
		} else {
			$this->_do_resize($width, $height, $width, $height);
		}		
	 ...
	 * @param   boolean  $duplicate  add dulicate watermark or not
	 * @return  $this
	 * @uses    Image::_do_watermark
	 */
	public function watermark($watermark, $offset_x = NULL, $offset_y = NULL, $opacity = 100, $duplicate = FALSE)
	{
		if ($offset_x === NULL) {
			// Center the X offset
		...
		// add duplicate watermark or not
		if ($duplicate) {
			$cnt_x = ceil($this->width / $watermark->width);
			$cnt_y = ceil($this->height / $watermark->height);
			for ($i=0; $i < $cnt_x; $i++) {
				for ($j=0; $j < $cnt_y; $j++) { 
					$offset_x = $watermark->width * $i;
					$offset_y = $watermark->height * $j;
