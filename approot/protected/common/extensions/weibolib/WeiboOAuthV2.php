...
use common\extensions\weiboauth\WeiboOAuthException;
...
class WeiboOAuthV2 {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new WeiboOAuthException("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new WeiboOAuthException("get access token failed." . $token['error']);
		}
		return $token;
	}
