...
Yii::import('ext.ares.AresDataColumn');
 ...
 * @since 1.1
 */
class AresGridView extends CBaseListView
{
	 ...
	 * where "type" and "header" are optional. A {@link AresDataColumn} instance will be created in this case,
	 * whose {@link AresDataColumn::name}, {@link AresDataColumn::type} and {@link AresDataColumn::header}
	 ...
	 * the 'class' element specifies the column class name (defaults to {@link AresDataColumn} if absent).
	 * Currently, these official column classes are provided: {@link AresDataColumn},
	 ...
	 * This differs from {@link nullDisplay} in that {@link nullDisplay} is only used by {@link AresDataColumn} to render
	 * null data values.
	 * @since 1.1.7
	 */
	public $blankDisplay='&nbsp;';
	 ...
	 * Note that in order to show an input field for filtering, a column must have its {@link AresDataColumn::name}
	 * property set or have {@link AresDataColumn::filter} as the HTML code for the input field.
	...
	/**
	 * Creates a {@link AresDataColumn} based on a shortcut column specification string.
	 * @param string $text the column specification string
	 * @return AresDataColumn the column instance
	 */
	protected function createDataColumn($text)
	{
		if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$text,$matches))
			throw new CException(Yii::t('zii','The column must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
		$column=new AresDataColumn($this);
