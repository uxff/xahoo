...
/**
 * AresMailer class - wrapper for PHPMailer
 * Yii extension for sending emails using views and layouts
 *
 *  How to use:
 * 
 *   //$objMailer = new AresMailer();
 *   $objMailer = new AresMailer('newsletter', array('message' => 'Hello World', 'name' => 'Bruce', 'description' => 'Welcome into this page.'));
 *   //set properties
 *   $objMailer->setTo('liutao@fangfull.com');
 *   $objMailer->setSubject('test subject');
 *   $objMailer->setBody('TEST BODY ing...');
 *
 *   if( $objMailer->send() ) {
 *       echo 'Send Mail Success....<BR>';
 *   } else {
 *       echo 'Error while sending email: <BR>';
 *       echo $objMailer->getError();
 *   }
 *
 *
 * @package AresMailer
 ...
 * AresMailer class - wrapper for PHPMailer
 */
class AresMailer extends PHPMailer {
         ...
         */
        protected $viewPath = 'application.common.views.mail';
        protected $layoutPath = 'application.common.views.mail.layouts';
        ...
        const CONFIG_PARAMS = 'AresMailer'; //Define the key of the Yii params for the config array
        ...
        public function __construct($view = '', $data = array(), $layout = '') {
                //initialize config
                $config = require(Yii::getPathOfAlias('application.common.config') . DIRECTORY_SEPARATOR . self::CONFIG_FILE);
                if (isset(Yii::app()->params[self::CONFIG_PARAMS])) {
                       $config1 = Yii::app()->params[self::CONFIG_PARAMS];
                ...
                // $this->setFrom($config['Username'], 'No-Reply');
                //set view
                $this->setView($view);
                //set data
                $this->setData($data);
