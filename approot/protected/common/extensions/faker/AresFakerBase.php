...
class AresFakerBase {
    ...
    public static function randomNumber($nbDigits = null, $strict = false)
    {
        ...
        if (null === $nbDigits) {
            $nbDigits = self::randomDigitNotNull();
        }
    ...
    public static function randomFloat($nbMaxDecimals = null, $min = 0, $max = null)
    {
        if (null === $nbMaxDecimals) {
            $nbMaxDecimals = self::randomDigit();
        }
        if (null === $max) {
            $max = self::randomNumber();
        }
        if ($min > $max) {
            $tmp = $min;
            $min = $max;
    ...
    public static function randomElement($array = array('a', 'b', 'c'))
    {
        ...
        $elements = self::randomElements($array, 1);
        return $elements[0];
    }
    /**
    ...
    /**
     * Returns a random datetime from parameters
     * 
     * @param  string $start 起始时间
     * @param  string $end   结束时间
     * @return string        随机时间
     */
    public static function randomDatetime($start = '', $end = '')
    {
        if (!empty($start) && !empty($end)) {
            $min = min(strtotime($start), strtotime($end));
            $max = max(strtotime($start), strtotime($end));
            return date('Y-m-d H:m:s', mt_rand($min, $max));
        } else {
    ...
    {
        if (is_array($arg)) {
            return self::shuffleArray($arg);
        }
        if (is_string($arg)) {
            return self::shuffleString($arg);
        }
    ...
    public static function shuffleString($string = '', $encoding = 'UTF-8')
    {
        ...
        return join('', self::shuffleArray($array));
    }
    /**
    ...
    public static function numerify($string = '###')
    {
        ...
        if ($nbReplacements = count($toReplace)) {
            ...
            while ($i < $nbReplacements) {
                ...
                $numbers .= str_pad(self::randomNumber($size), $size, '0', STR_PAD_LEFT);
                $i += $size;
            }
        ...
        $string = preg_replace_callback('/\%/u', 'self::randomDigitNotNull', $string);
        return $string;
    }
    /**
    ...
    {
        return preg_replace_callback('/\?/u', 'self::randomLetter', $string);
    }
    /**
    ...
    {
        return preg_replace_callback('/\*/u', 'self::randomAscii', $string);
    }
    /**
...
*/
        ...
        //5.2版本使用
        // [12]{1,2} becomes [12] or [12][12]
        $regex = preg_replace_callback('/(\[[^\]]+\])\{(\d+),(\d+)\}/', 
        ...
        $regex = preg_replace_callback('/\[([^\]]+)\]/', 
            create_function('$matches', "
            return '[' . preg_replace_callback('/(\w|\d)\-(\w|\d)/', 
                create_function('$range', \"
                return join(range($range[1], $range[2]), '');
            \"), $matches[1]) . ']';
            "), 
            $regex);
