 ...
 *
 *       $res = $objRestClient->doGet('http://hifang.local.com/xqsjfqpc.php', array('r'=>'api/getHotProject', 'num'=>2));
 *       $result = CJSON::decode($res);
 */
class AresRESTClient extends CComponent
{
	public $supported_formats = array(
    	...
    	public function doGet($uri, $params = array(), $format = NULL)
    	{

    		// 增加访问签名限制
    		//$sign = AresUtil::signURLParameters($params);
    		//$params['sign'] = $sign;
    	...
    	public function doPost($uri, $params = array(), $format = NULL)
    	{
    		 
    		// 增加访问签名限制
    		//if(!isset($params['sign'])){
    		//	$sign = AresApiUtil::signURLParameters($params);
    		//	$params['sign'] = $sign;
    		//}
    	...
    	public function doPut($uri, $params = array(), $format = NULL)
    	{
    		return $this->_call('put', $uri, $params, $format);
    	}


    	public function doDelete($uri, $params = array(), $format = NULL)
    	{
    	...
    	private function _call($method, $uri, $params = array(), $format = NULL)
    	{
            ...
            $this->_origin_response = $response;
