...
class SmsManager extends CApplicationComponent 
{
     ...
     */
    public function sendSMS($mobile,$content,$sendTime = null)
    {
        // 判断手机号是否为空
        if (empty($mobile)) {
            $message = '无效的手机号';
        ...
        if (empty($message)) {
            ...
            if (empty($checkResult)) {
                ...
                $this->saveSended($mobile,$content.$this->_sign,$this->_sendTime,$result,2);//日志
                // 判断错误信息
    ...
    /**
     * 保存日志 //记录转移到已发送表 is_send>1|短信发送成功，2|系统跳过短信发送
     * @param unknown $smsinfo
     * @param string $is_Send
     * @return boolean
     */
    function saveSended($mobile,$info,$sendtime,$result,$is_Send='1'){
        $desc=array(
            "IP"=>Yii::app()->request->userHostAddress,
            "pageurl"=>Yii::app()->request->hostInfo.Yii::app()->request->getUrl(),
            "urlReferrer"=>Yii::app()->request->urlReferrer,
        );
        if(!$sendtime){$sendtime=date("Y-m-d H:i:s",time());}
        
        $result=Yii::app()->UCenterDb->createCommand()->insert('fq_order_sms_sended', array(
        "sms_id"=>0,
        "sms_mobile"=>$mobile,
        "sms_message"=>$info,
        "smstpl"=>'reg_fanghu',
        "pre_send_time"=>date("Y-m-d H:i:s",time()),
        "is_send"=>$is_Send,//状态直接跳过
        "send_time"=>$sendtime,
        "send_error_times"=>0,
        "order_id"=>0,
