...
<script>    
    var valid_area = '{$poster['valid_area']}';
</script>
{literal}
<script type="text/javascript">
    $(function (){
        ...
        $("#valid_area").val(valid_area);
        var inp = $('.select_rel');
        if(valid_area!= null&&inp.length!=0){
            valid_area = valid_area.split(',');
            
            var arr = [];
            for(o in city){
                for(var i=0; i<valid_area.length; i++){
                    if(valid_area[i]==city[o].name){
                        arr.push(city[o].name);
                    }
                }
            }
            inp.val(arr);
            $('.content label').each(function(){
                ...
                $.each(arr,function(){
                    if(this == _txt){
                    _this.prev('input').attr("checked",'checked');
                   }
                })
            })
        }
    ...
    jQuery(function ($) {
        ...
        new uploadFile({
        	...
        	'formData':{
              ...
              thumbnailWidth : 750 * (window.devicePixelRatio ? window.devicePixelRatio: 1),
