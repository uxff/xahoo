<?php
class EventLogModel extends EventLogModelBase
{
        ...
        public function relations() {
                $curRelations = array(
                    'event_tpl' => array(self::HAS_ONE, 'EventTplModel', '', 'on' => 'event_tpl.event_id = t.event_id'),
                );
        ...
        public function mySearch($condition = array(), $page = 1, $pageSize = 10)
        {
               // @todo Please modify the following code to remove attributes that should not be searched.
                $criteria = $this->getBaseCDbCriteria();
                if ($condition)
                foreach ($condition as $condKey=>$condVal) {
                    $criteria->addCondition('t.'.$condKey, $condVal);
                }
                //为$criteria新增设置
                $count = $this->count($criteria);
                $pager = new CPagination($count);
                $pager->currentPage = $page-1;
                $pager->pageSize = !empty(Yii::app()->params['pageSize'])?Yii::app()->params['pageSize']:$pageSize;
         ...
         * @return EventLogModel the static model class
         */
