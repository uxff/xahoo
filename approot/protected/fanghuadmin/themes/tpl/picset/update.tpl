<div class="page-content-area">
        <div class="page-header">
            <h1> <a href="fanghuadmin.php?r=picset">图库管理</a> <small> <i class="ace-icon fa fa-angle-double-right"></i> 编辑 </small> </h1><br />
        ...
        <div class="row">
                <div class="col-xs-12"> 
                        ...
                        <form class="form-horizontal" id="picset-form" role="form" action="fanghuadmin.php?r=picset/update&id={$model[$primaryKey]}" method="POST">
                ...
                <!--已上传循环-->
                {foreach from=$dataObj.pics key=k item=picObj}
                <li id="pic-element-{$picObj.id}">
                <div class="form-group">
                    <label class="col-sm-2 control-label no-padding-right" for=""></label>
                    <div class="col-sm-7">
                        <img src="{$picObj.file_path}" style="width:250px;height:173px" />
                        <label>URL<input type="text" name="link_url[{$picObj.id}]" value="{$picObj.link_url}"/></label>
                        <label>权重<input type="text" name="weight[{$picObj.id}]" value="{$picObj.weight}"/></label>
                        <input type="hidden" name="pic_storage_id[{$picObj.id}]" value="{$picObj.id}"/>
                        <input type="button" value="删除图片" onclick="deletePic({$picObj.id})"/>
                        <input type="button" value="替换图片"/>
                    </div>
                </div>
                </li>
